...
description: "Improve inbox placement with Lettr by warming up domains, monitoring key metrics, maintaining list hygiene, and authenticating mail"
---
...
Network failures and platform retries can send the same email twice. Send an `Idempotency-Key` header derived from the business event; reusing the same key and payload within 24 hours returns the original result instead of sending again. Setting `metadata.idempotency_key` does not deduplicate:
...
See [Idempotency](/learn/sending/idempotency) for key rules and the retry pattern.
