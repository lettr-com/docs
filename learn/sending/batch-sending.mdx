---
title: Batch Sending
description: "Send the same email to up to 50 recipients per Lettr API call, personalize content with merge tags, and split larger lists into batches"
---
...
| Max requests per minute | See [Rate Limits](/api-reference/rate-limit) |

## Sending to Large Lists

For recipient lists larger than 50, chunk the list into batches and send each batch with a separate API call. Adding a short delay between batches helps you stay within [rate limits](/api-reference/rate-limit):
...
<AccordionGroup>
  ...
  <Accordion title="Use Webhooks for Delivery Status">
    With batch sends, polling the API for each email's delivery status is impractical. Set up webhooks to receive `message.delivery`, `message.bounce`, and `engagement.open` events, which let you track outcomes for every recipient asynchronously.
  </Accordion>
</AccordionGroup>
...
You can then use the `request_id` returned from each batch send to look up emails via the [Email History](/learn/sending/email-history) API. Metadata is also included in [webhook payloads](/learn/webhooks/introduction), allowing you to correlate delivery events with your batch data.
