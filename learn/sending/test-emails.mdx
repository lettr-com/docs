---
title: Test Emails
description: "Send test emails through Lettr's full delivery pipeline to check rendering and merge tags without affecting analytics or engagement"
---
...
- **[Sandbox API keys](/learn/api-keys/sandbox)** — a separate key type (`lttr_sandbox_` prefix) that redirects *every* email to your own inbox regardless of the recipients in the request, sends from a pre-verified sandbox domain, and has no billing impact. Best for local development, CI pipelines, and testing before your domain is verified.
...
An editor test email and an API send are not the same thing, and the differences explain most "my test looked different" reports:

| | Editor test email | API send with a template slug |
|-|-|-|
| **Content used** | The template as it currently looks in the editor, including unsaved changes | The **published** (active) version — see [Saving vs. Publishing](/learn/templates/versions#saving-vs-publishing) |
| **Merge tag values** | None — merge tags arrive empty | Whatever you pass in `substitution_data` |

Because an editor test carries no merge tag values, it cannot show you whether a merge tag link or button works. To verify those, send through the API with a [sandbox key](/learn/api-keys/sandbox) and real `substitution_data`, after publishing the version you want to test (or pass its number as `template_version`).
...
<AccordionGroup>
  ...
  <Accordion title="Test Links">
    Click every link in the test email to verify it resolves to the correct destination. Pay particular attention to links built with merge tags, unsubscribe links, and tracking-wrapped URLs. A link or button whose entire address is one merge tag (e.g., `{{tracking_url}}`) breaks at send time, because the full URL gets URL-encoded — keep `https://` and the fixed part of the address in the template instead. See [Links and URLs](/learn/templates/template-language#links-and-urls). Broken links in transactional emails erode user trust.
  </Accordion>
...
For automated integration tests or CI pipelines, the simplest option is a [sandbox API key](/learn/api-keys/sandbox): configure your staging environment with a sandbox key instead of a live one, and every email is automatically contained — recipients are overridden to the key owner's inbox, so no code changes are needed and nothing can reach customers.

Alternatively, you can keep a live key and redirect recipients yourself:
