...
<Warning>
  Because `transactional` defaults to `true`, every API send bypasses unsubscribe suppression unless you set it to `false`. A recipient who unsubscribed from your marketing email will still receive an API send that leaves the option at its default. Set `transactional: false` on marketing-shaped API sends (newsletters, promotions, product announcements) so suppressions are honoured.
</Warning>
