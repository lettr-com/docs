...
description: "Get started sending transactional and marketing emails with the Lettr API, including prerequisites, a basic send, and email options"
---
...
<Note>
  Don't have a verified domain yet? A [sandbox API key](/learn/api-keys/sandbox) lets you make real send requests immediately — every email is redirected to your own inbox and sent from a pre-verified sandbox domain, with no billing impact. It's the fastest way to try everything on this page before your domain is set up.
</Note>
...
<Warning>
  Because `transactional` defaults to `true`, every API send bypasses unsubscribe suppression unless you set it to `false`. A recipient who unsubscribed from your marketing email will still receive an API send that leaves the option at its default. If what you are sending through the API is marketing-shaped (a newsletter, a promotion, a product announcement), set `transactional: false` so suppressions are honoured.
</Warning>
