...
description: "Send one campaign in several languages. Each contact receives the version that matches their language, with its own subject, sender, unsubscribe footer, and web version."
---

A multilingual campaign is one campaign that goes out in more than one language. One template holds a Czech and an English version, one audience is selected, and every contact receives the language they asked for. There is no second campaign, no second segment, and no second report to reconcile.
...
<CardGroup cols={3}>
  <Card title="A template with languages" icon="languages" href="/learn/multilingual-campaigns/template-languages">
    Add languages to a template in the email editor. One is primary, the rest are secondary.
  </Card>
  <Card title="A contact language property" icon="user-tag" href="/learn/multilingual-campaigns/contact-language">
    A property on each contact that says which language they want. Designate one, or let Lettr find it by name.
  </Card>
  <Card title="The campaign" icon="bullhorn" href="/learn/multilingual-campaigns/campaign-setup">
...
1. A contact with `cs` gets the Czech version. A contact with `cs-CZ` or `czech` gets it too. Matching is forgiving; see [how values are matched](/learn/multilingual-campaigns/contact-language#how-values-are-matched).
...
3. Each recipient gets their language's content, subject, sender and reply-to, an unsubscribe footer translated into their language, and a web version in the same language.
...
<Steps>
  <Step title="Add a second language to a template">
    Open a marketing template in the editor and use the **Multilingual** control in the top bar to add a language. Translate the text. See [Template languages](/learn/multilingual-campaigns/template-languages).
  </Step>
...
| Unsubscribe footer | Translated into the recipient's language (19 languages, English for anything else) |
