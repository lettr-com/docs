---
title: "Email Events"
description: "Use the Events dashboard to track the delivery lifecycle of every email you send, from injection through opens, clicks, and bounces."
---
...
| **Event type** | Show only one type, such as Bounce or Click — see [Event Types](/learn/events/event-types) |
| **Recipient email address** | One complete address, matched exactly. Partial addresses and domains don't match. |

The Events page **cannot** filter by template, subject line, or email content. To find the emails sent from one template, search by the recipient you sent to and narrow the date range to when the send happened; the subject column then identifies the message.

<Note>
  Events also don't show the merge tag values (`substitution_data`) an email was sent with. Lettr does not store those values — the API logs record only how many were sent (see [Truncated Fields](/learn/logs/introduction#truncated-fields)). If you need to know later which values a message was sent with, attach the important ones as [metadata](/learn/sending/metadata), which is kept and shown on the Message Details page, or log them in your own application.
</Note>
...
<Tip>
  There's a third way to explore events: ask an AI assistant. The [remote Lettr MCP server](/learn/mcp/setup) has a `browse_email_events` tool, so a prompt like *"show me all bounces from the last week"* in Claude.ai or ChatGPT queries the same event data without opening the dashboard.
</Tip>
