---
title: API Logs
description: "Use the Logs page to view every API request made to Lettr, with method, path, status, and IP, to monitor and debug your integration."
---
...
- **Request Body** - The JSON payload sent to the API, with some fields shortened (see [Truncated Fields](#truncated-fields))
...
<Note>
  Truncation only affects the logs. The actual email sent contains the full content.
</Note>

<Warning>
  The shortened values are not kept anywhere else. In particular, the merge tag values in `substitution_data` are used to build the email and then discarded — they often contain personal data, so Lettr does not store them, and neither Logs nor [Events](/learn/events/introduction) can show which values an email was sent with. If you need that for debugging, log the payload in your own application, or pass key identifiers as [metadata](/learn/sending/metadata), which is visible on the Message Details page.
</Warning>
...
<Tip>
  [Adamko](/learn/ai-assistant/troubleshooting-and-diagnostics#integration--api-problems) can run this analysis for you: ask *"why are my API requests failing?"* and he samples recent logs, cross-references your API keys and webhooks, and returns the issues ordered by severity with sample failed requests as evidence.
</Tip>
