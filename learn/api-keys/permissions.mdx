...
When creating an API key, you assign a permission level that controls which API endpoints the key can access. Lettr supports three permission levels: **Full Access**, **Sending Only**, and **Custom**.
...
Custom keys let you select individual scopes for fine-grained access control. Instead of choosing a preset, you pick exactly which operations the key can perform.
...
4. Select the permission level:
   ...
   - **Custom** — Select individual scopes from the checklist
