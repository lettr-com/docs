...
description: "Test your Lettr integration with sandbox API keys that redirect every email to your own inbox, with no domain setup or billing impact"
---
...
- `GET /api/emails/scheduled/{requestId}` — Get scheduled email details
- `DELETE /api/emails/scheduled/{requestId}` — Cancel a scheduled email
...
Sandbox keys also work as the password for [SMTP sending](/quickstart/smtp/introduction). SMTP messages are processed through the same pipeline as API sends, so the same sandbox behavior applies: recipients are redirected to your inbox and the sender domain is replaced with the sandbox domain.

<Note>
  Sandbox sends run on an isolated subaccount, so they do **not** trigger your team's webhooks. To verify sandbox activity, use the **Sandbox** toggle on the Events page in the dashboard instead.
</Note>
