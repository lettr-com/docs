...
Lettr API keys support two permission levels to help you follow the principle of least privilege:

| Permission Level | Description | Use Case |
|-----------------|-------------|----------|
| **Full Access** | Complete access to all API endpoints including email sending, domain management, webhooks, and templates | Server-side applications requiring full platform control |
| **Sending Only** | Limited to email sending operations only | Applications that only need to send transactional emails |

<Tip>
  Use **Sending Only** keys whenever your application only needs to send emails. This reduces the potential impact if a key is compromised.
</Tip>
...
|----------|--------|-------------|---------------------|
| `/api/emails` | POST | Send an email | Sending Only or Full Access |
| `/api/emails` | GET | List sent emails | Full Access |
| `/api/emails/{requestId}` | GET | Get email details | Full Access |
| `/api/templates` | GET | List templates | Full Access |
| `/api/templates` | POST | Create a template | Full Access |
| `/api/templates/{slug}` | GET | Get template details | Full Access |
| `/api/templates/{slug}` | PUT | Update a template | Full Access |
| `/api/templates/{slug}` | DELETE | Delete a template | Full Access |
| `/api/templates/{slug}/merge-tags` | GET | Get template merge tags | Full Access |
| `/api/projects` | GET | List projects | Full Access |
| `/api/domains` | GET | List domains | Full Access |
| `/api/domains` | POST | Create a domain | Full Access |
| `/api/domains/{domain}` | GET | Get domain details | Full Access |
| `/api/domains/{domain}/verify` | POST | Verify a domain | Full Access |
| `/api/domains/{domain}` | DELETE | Delete a domain | Full Access |
| `/api/webhooks` | GET | List webhooks | Full Access |
| `/api/webhooks/{webhookId}` | GET | Get webhook details | Full Access |
