...
DMARC (Domain-based Message Authentication, Reporting, and Conformance) is an email authentication protocol that protects your domain from being used in phishing and spoofing attacks. It tells receiving email servers what to do when messages fail SPF or DKIM authentication.
...
Lettr automatically configures SPF and DKIM to align with DMARC:

### DKIM Alignment

- Lettr signs emails with your domain's DKIM key
- The `d=` parameter in the DKIM signature matches your sending domain
- Supports both relaxed and strict alignment

### SPF Alignment

- Lettr uses a return-path domain that aligns with your sending domain
- The bounce CNAME record enables SPF alignment
- Supports both relaxed and strict alignment
