...
<Info>
  Lettr uses CNAME records for DKIM to simplify key management. This allows automatic key rotation without requiring you to update DNS records.
</Info>
...
You can have multiple DKIM keys for different purposes (e.g., different email services). Each key uses a different selector:

| Service | Selector | DNS Record Host |
|---------|----------|-----------------|
| Lettr | `lettr` | `lettr._domainkey` |
| Google | `google` | `google._domainkey` |
| Mailchimp | `k1` | `k1._domainkey` |

<Tip>
  Unlike SPF, you can have multiple DKIM records. Each service uses its own selector, so they don't conflict.
</Tip>

## DKIM Key Rotation

Lettr automatically rotates DKIM keys to maintain security. Because you use a CNAME record pointing to `dkim.lettr.dev`, key rotation happens seamlessly without any DNS changes on your end.
...
if (domain.dkimStatus === 'verified') {
  console.log('DKIM is correctly configured!');
} else {
  console.log('DKIM status:', domain.dkimStatus);
  console.log('Expected record host:', domain.dkimRecordHost);
  console.log('Expected record value:', domain.dkimRecordValue);
}
```
