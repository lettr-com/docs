...
| TXT | `lettr._domainkey` | `v=DKIM1; k=rsa; p=...` | DKIM signing |
| TXT | `_dmarc` | `v=DMARC1; p=none; rua=mailto:dmarc@example.com` | DMARC policy |
| CNAME | `bounce` | `bounce.lettr.dev` | Bounce handling |

<Note>
  The exact values for DKIM will be provided when you add your domain. Copy them exactly as shown.
</Note>

## Domain Status

Each domain has a verification status for its DNS records:

| Status | Description |
|--------|-------------|
| `pending` | DNS records not yet detected |
| `verified` | Records verified and active |
