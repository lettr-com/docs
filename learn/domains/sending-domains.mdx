...
```json
{
  ...
  "data": {
    ...
    "dkim": {
      "record_type": "TXT",
      "hostname": "lettr._domainkey.example.com",
...
SPF (Sender Policy Framework) authorizes Lettr to send emails on your behalf.
...
If you already have an SPF record, merge them into a single record. SPF allows only one TXT record per domain.

**Before (two separate records - invalid):**
```
...
|-------|-------|
| Type | TXT |
| Host/Name | `lettr._domainkey` |
| Value | `v=DKIM1; k=rsa; p=...` (provided by Lettr) |

<Note>
  The DKIM value is unique to your domain and will be provided when you add your domain. Copy it exactly as shown since it is a long string.
</Note>

### CNAME Record (Return-Path)

The CNAME record configures bounce handling for improved deliverability.
...
| Status | Description |
|--------|-------------|
| `verified` | Record found and valid |
...
<AccordionGroup>
  ...
  <Accordion title="DKIM verification failing">
    ...
    - Check that the hostname is exactly `lettr._domainkey` (not `lettr._domainkey.example.com` in the host field)
    - Some DNS providers require you to remove quotes from the value
  </Accordion>

  <Accordion title="SPF record issues">
    - You can only have one SPF record per domain. Merge multiple includes into one record.
    - Ensure the record starts with `v=spf1`
    - Check that `include:spf.lettr.dev` is present
  </Accordion>
