...
<Tip>
  If verification keeps failing, [Adamko](/learn/ai-assistant/troubleshooting-and-diagnostics#domain-verification-problems) can diagnose it for you: ask *"why isn't my domain verifying?"* and he runs live DNS lookups, compares each record against what Lettr expects, and tells you exactly which record is wrong and how to fix it.
</Tip>

<Note>
  You don't have to wait for DNS propagation or domain approval to start building. Create a [sandbox API key](/learn/api-keys/sandbox) and test your sending integration right away — sandbox sends use a pre-verified sandbox domain and deliver to your own inbox, so no verified sending domain is required.
</Note>
