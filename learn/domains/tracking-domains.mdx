...
description: "Set up a custom tracking domain for click links and open pixels, served from your brand over HTTPS with automatically provisioned SSL"
---
...
<Warning>
  If your DNS is managed by Cloudflare, the CNAME record must be set to **DNS only** (grey cloud). With the proxy enabled (orange cloud), the hostname returns Cloudflare's A records instead of the CNAME, and Lettr rejects verification with a "hostname is returning A records instead of the required CNAME" error. See the [Cloudflare guide](/knowledge-base/dns-guides/cloudflare) for details.
</Warning>
