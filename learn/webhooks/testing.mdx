...
description: "Test Lettr webhooks locally with ngrok tunnels and verify event handling before deploying your endpoint to production."
---
...
<Note>
  Emails sent with a [sandbox API key](/learn/api-keys/sandbox) run on an isolated subaccount and do **not** trigger your team's webhooks. To generate real webhook events, send with a live API key — [test emails](/learn/sending/test-emails) work well for this, since their events still fire while staying out of your analytics.
</Note>
...
5. **Configure webhook in Lettr**: Go to **Webhooks** in the sidebar and create a new webhook pointing to your ngrok URL (e.g., `https://abc123.ngrok.io/webhooks/lettr`).
