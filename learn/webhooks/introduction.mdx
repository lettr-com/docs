...
| `engagement.open` | Track engagement metrics and trigger follow-up sequences |
| `engagement.click` | Measure link performance and user interest |
...
- `engagement.open` - Recipient opened the email
- `engagement.initial_open` - First open of the email
- `engagement.click` - Recipient clicked a link
- `engagement.amp_open` - Recipient opened an AMP email
- `engagement.amp_initial_open` - First open of an AMP email
- `engagement.amp_click` - Recipient clicked a link in an AMP email
...
<Note>
  A webhook name can be at most 24 characters. An update must change at least one of `name`, `url`, `auth_type`, `events` or `active`; an empty update returns `422`. To change credentials, send them together with `auth_type` (see [Authorization](/learn/webhooks/authorization)).
</Note>
