...
The Lettr MCP server provides 16 tools organized into five categories: email sending, template management, configuration, analytics, and monitoring. This reference documents each tool's parameters and behavior.
...
Lists all webhooks configured for your team, including target URL, enabled status, event types, authentication type, and delivery health indicators (last status, last success, and last failure timestamps).
...
| **Templates** | `list_templates`, `get_template`, `create_template`, `update_template` | List, view, create, and update email templates |
...
| **Analytics** | `get_analytics`, `list_template_stats` | View delivery metrics and per-template statistics |
