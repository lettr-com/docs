...
Ask Adamko for an API key or a sending domain in any conversation, at any point in your account's life, and he creates it on the spot:
...
Registering a domain in Lettr never touches your registrar, so there is nothing for Adamko to log in to. If a domain is already registered by another team or is on a blocklist, he relays the actual reason. Ask to be walked through the domain page instead and he hands off to the [guided walkthrough](/learn/ai-assistant/guided-tours-and-nudges).
...
<Note>
  **If your website can't be read**, brand detection stops with a plain-language explanation naming the host and what to try next. Typical causes are bot protection returning a 403, a mistyped address, a host that doesn't respond, or a page that serves an empty JavaScript shell to crawlers. The setup wizard offers to try another URL and will not scaffold emails until a brand kit has completed.
</Note>
