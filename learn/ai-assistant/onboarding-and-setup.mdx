...
## Keys and Domains from Ordinary Chat

The setup flow is not the only place Adamko can create things. Ask him in any conversation, at any point in your account's life:

- *"Create an API key called production"* creates the key and shows the secret once, in the same copy-once card the setup flow uses.
- *"Add mail.example.com as a sending domain"* registers the domain and shows the DNS records to add.

Registering a domain in Lettr never touches your registrar, so there is nothing for Adamko to log in to. If a domain is already registered by another team or is on a blocklist, he relays the actual reason rather than guessing. And if you would rather be walked through the domain page than have it done for you, say so and he hands off to the [guided walkthrough](/learn/ai-assistant/guided-tours-and-nudges) instead.
