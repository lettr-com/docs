...
```json
{
  ...
  "data": {
    ...
    "mx_records": [
      { "priority": 5, "hostname": "rx1.sparkpostmail.com" },
      { "priority": 10, "hostname": "rx2.sparkpostmail.com" },
      { "priority": 20, "hostname": "rx3.sparkpostmail.com" }
    ],
...
MX priority determines the order email servers attempt delivery:

| Priority | Server | Purpose |
|----------|--------|---------|
| 5 (highest) | `rx1.sparkpostmail.com` | Primary mail server |
| 10 | `rx2.sparkpostmail.com` | Secondary fallback |
| 20 (lowest) | `rx3.sparkpostmail.com` | Tertiary fallback |

Lower numbers indicate higher priority. If the primary server is unavailable, sending servers automatically try the next server in order.
...
const response = await fetch(
  'https://app.lettr.com/api/domains/inbound/mail.example.com/verify',
  {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer le_xxxxxxxxxxxx'
