---
title: Contacts
description: "Manage individual contacts in your Lettr audience, including their five statuses, custom properties, memberships, and activity history"
---
...
A pause is not a sixth status. A contact who pauses marketing email from the [preferences page](/learn/audience/email-preferences) stays `subscribed` but carries a `paused_until` date, and campaigns skip them until that date passes. Paused contacts show a violet **Paused** badge in Audiences, can be isolated with the **Paused** filter, and appear in exports and on the API with their `paused_until` value. Pauses are set by recipients only; they can't be created or edited from the app or the API.
...
<Tip>
  Importing from another system or syncing contacts from your own database? The [Contacts API](/api-reference/audience/bulk-create-contacts) can create or update many contacts in a single request, so you don't have to call the create endpoint once per contact.
</Tip>
...
<Card title="Try it in the app" icon="wand-magic-sparkles" href="https://app.lettr.com/audience/contacts?guide=">
...
One property per team can hold each contact's language. Tick **Use as communication language** when you create or edit the property, and it shows a **Communication language** badge in the list. Campaigns built on a template with more than one language read the recipient's language from this property and send each person the matching version. The hosted [preferences page](/learn/audience/email-preferences) renders in that language too. If no property is designated, Lettr falls back to a property named `communication_language`, `communication_lang`, `language`, `lang` or `locale`. See [Contact language](/learn/multilingual-campaigns/contact-language) for the matching rules.
...
Topic changes made over the API, including the [bulk subscribe](/api-reference/audience/bulk-subscribe-contacts-to-topics) and [bulk unsubscribe](/api-reference/audience/bulk-unsubscribe-contacts-from-topics) endpoints and row-level `opt_out` entries in a [bulk contact import](/api-reference/audience/bulk-create-contacts), are logged as **Topic opted in** / **Topic opted out** events, so consent history stays complete however the change was made.
