...
description: "Bulk import contacts into Lettr from a CSV file with guided column mapping and duplicate handling, and export your audience to a file"
---
...
<Tip>
  Syncing contacts from your own system instead of a CSV? The [bulk create contacts](/api-reference/audience/bulk-create-contacts) endpoint creates or updates up to 1000 contacts in a single API request — each row can carry its own properties, list memberships, and topic subscriptions — with the same skip-or-update behavior the CSV flow offers.
</Tip>
...
- **Importing a language column?** Map it to a property named `language`, `lang` or `locale` (or one you've marked **Use as communication language**) and [multilingual campaigns](/learn/multilingual-campaigns/contact-language) will route each contact to their language with no further setup. Values like `de`, `de_DE` and `German` all work.
...
<Tip>
  With [Adamko](/learn/ai-assistant/audience-and-imports) enabled, you can go further than one-click mapping: drop your CSV straight onto his chat drawer to start the import, ask him to **review the file** for invalid addresses, duplicates, and consent risks before you run it, and let him **create missing properties and map skipped columns** for you — always behind a confirm-first preview. He never reads the raw file; it goes directly to the importer.
</Tip>
...
The CSV contains one row per matching contact. Columns include the email address, status, `paused_until` (the end of an active [pause](/learn/audience/email-preferences), empty otherwise), every custom property defined for your team, and the timestamps for when the contact was created and last updated.
