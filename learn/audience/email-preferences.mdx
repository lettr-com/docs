...
Every `{{unsubscribe_link}}` in a campaign opens a hosted **Manage email preferences** page instead of a one-click "you're unsubscribed" screen. Recipients see where they stand, choose which topics they want, pause marketing email for a while, or leave entirely. The page is built in, so there is nothing to host or design yourself.
...
**Coming back** is the same button in reverse. An unsubscribed recipient ticks the topics they want and presses **Update preferences**. Their status returns to `subscribed` with exactly those topics. Resubscribing is always an explicit act; nothing else on the page, a pause included, can turn an opt-out back into an opt-in.

**Pausing** keeps the contact `subscribed` but sets a `paused_until` date. Campaigns skip the contact until that date passes, after which sends resume on their own. There is no scheduled job involved, so a pause cannot fail to expire. Pausing is only available to subscribed contacts.
...
A pause is owned by the recipient. Contacts cannot be created or edited into a paused state from the app or the API. If you need to stop mail to someone yourself, change their [status](/learn/audience/contacts#contact-status) instead.
...
- **The page never fails closed.** A malformed or missing optional parameter is dropped rather than turned into an error, because refusing to render an unsubscribe page is worse than ignoring a bad query string.
