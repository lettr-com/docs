...
You can link a campaign to one of your [templates](/learn/templates/introduction) instead of writing content directly. To link, choose **Use template** in the Compose step and pick a marketing template — one that lives in your Marketing mode.

If the design you want only exists as a transactional template, use its **Copy to Marketing** action to create a marketing copy (see [Transactional vs. Marketing Templates](/learn/templates/projects#transactional-vs-marketing-templates)). A template's mode is fixed once it is created, so copying is the way to move a design across — there is no in-place switch.

<Tip>
  Creating templates through the API? Pass `purpose: "campaign"` and the template arrives in Marketing mode ready to link — no copy step. This also applies to templates imported from Topol. See [Choosing the mode over the API](/learn/templates/projects#choosing-the-mode-over-the-api).
</Tip>
...
If the template has more than one language, linking it makes the campaign a [multilingual campaign](/learn/multilingual-campaigns/introduction). Each recipient receives the language that matches their contact's language property, and a **Per-language subject and sender** section appears below the sender fields so you can translate the subject or change the sender per language. See [Campaign setup](/learn/multilingual-campaigns/campaign-setup) for the full walkthrough.
...
Every campaign you send through Lettr must include an unsubscribe mechanism. Lettr exposes this as a merge tag: `{{unsubscribe_link}}`. At send time it's replaced with a unique signed per-recipient URL that opens the hosted [Manage email preferences](/learn/audience/email-preferences) page, where the recipient can untick topics, pause marketing email for 30, 60, or 90 days, or unsubscribe from everything. A full unsubscribe marks the contact as `unsubscribed` and fires the corresponding webhook event.
...
In a [multilingual campaign](/learn/multilingual-campaigns/sending-and-results#the-unsubscribe-footer), the default footer matches the language version each recipient receives, so a German recipient sees "Abmelden" rather than "Unsubscribe".
...
When the campaign links a template with more than one language, the test box has a language picker. Send one test per language to check each translated subject and sender. See [Sending a test email](/learn/multilingual-campaigns/campaign-setup#sending-a-test-email) in the multilingual guide.
