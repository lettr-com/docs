...
<Tip>
  These lifecycle actions are also available via the API. Use the Campaigns API to [send a campaign now](/api-reference/campaigns/send-a-campaign-now), [schedule a campaign](/api-reference/campaigns/schedule-a-campaign), or [unschedule](/api-reference/campaigns/unschedule-a-campaign) one that hasn't started yet.
</Tip>
...
<Note>
  Scheduling locks the content. If the campaign links a template with more than one language, Lettr saves every language version at this moment and sends from that copy, so later edits to the template don't change what goes out. Unscheduling releases the copy; rescheduling takes the template as it is then. See [Content is locked when you schedule](/learn/multilingual-campaigns/sending-and-results#content-is-locked-when-you-schedule).
</Note>
