...
description: "Configure the Lettr visual editor defaults for your team, including custom fonts, brand colors, merge tag groups, language, and test emails."
---
...
### Merge Tags and the Editor Preview

The merge tags you register here are also the only ones the editor's **Merge tags - Preview** panel knows about. That panel lists the registered tags used in the current template and lets you enter sample values for them.

You can still type any merge tag straight into your content — `{{TOURNAMENT_NAME}}` works at send time whether or not it is registered. But unregistered tags never show up in the preview panel, so a template that uses only hand-typed tags opens an **empty** preview panel. If that happens, add the tags here, save, and reopen the template. See [Previewing merge tags](/learn/templates/topol-editor#previewing-merge-tags).

<Note>
  Register the tag with the same spelling and capitalization you use in the template and in your API calls. Merge tag names are case-sensitive, and the value generated here is uppercase (`FIRST_NAME`).
</Note>
