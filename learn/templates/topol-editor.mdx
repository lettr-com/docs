...
Images are uploaded through the integrated **File Manager**, which provides folder organization, search, and access to the Pexels stock image library. Uploaded images are stored in your team's [Storage Domain](/knowledge-base/dns-guides/storage-domains) and are available across all templates.
...
The editor automatically saves template state at regular intervals. Autosave history provides access to previous versions for comparison or restoration.

Autosaves are separate from manual saves. Manual saves create the canonical version of the template that is used when the template is referenced for sending.
...
Images are hosted on your team's configured [Storage Domain](/knowledge-base/dns-guides/storage-domains) and served from your custom domain.
