...
Deleting a domain clears its health alert from the dashboard and the notification center at the same time. A **Check domain** link from an older alert email or notification that points at a domain you have since deleted lands on the domain list with an explanation.
