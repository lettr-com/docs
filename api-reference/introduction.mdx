---
title: Introduction
description: "Complete reference for the Lettr REST API, with resource-oriented URLs, JSON requests and responses, Bearer authentication, and HTTP codes."
---
...
To disable tracking through the HTTP API, set `options.open_tracking` and `options.click_tracking` to `false` in the JSON request body. These options work independently; omitting either keeps that setting enabled.

The `X-Lettr-Track-Opens` and `X-Lettr-Track-Clicks` headers control tracking only when sending over SMTP. For API requests, use the `options` fields rather than HTTP headers or the custom email `headers` object. See [Tracking](/learn/sending/tracking) for API and SMTP examples.
