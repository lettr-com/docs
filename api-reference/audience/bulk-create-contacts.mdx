...
By default (`update_existing: false`) existing contacts keep their properties, but they are still attached to the requested lists. Set `update_existing: true` to merge properties instead — submitted keys overwrite, absent keys are preserved — and to let an `opt_out` drop an existing subscription.
