---
title: Laravel
description: "Configure Laravel's built-in mail system to send emails through Lettr over SMTP using environment variables, no package required."
---
...
Add a `headers()` method to your Mailable to disable tracking for that message. Import `Headers` at the top of the file and place the method inside your existing Mailable class:
...
If your Mailable already defines `headers()`, add these entries to its `text` array. Use the strings `'true'` or `'false'`, and add each header only once. Omitting either header leaves that tracking setting enabled by default.

See [SMTP tracking headers](/learn/sending/tracking#smtp-tracking-headers) for validation rules and how the settings apply to each message.
