---
title: PHPMailer
description: "Send emails through Lettr using PHPMailer over SMTP in any PHP project, including WordPress, custom apps, and legacy codebases."
---
...
Open and click tracking are enabled by default. Add these headers before calling `$mail->send()` to disable both for a message:

```php
$mail->addCustomHeader('X-Lettr-Track-Opens', 'false');
$mail->addCustomHeader('X-Lettr-Track-Clicks', 'false');
```

Use the strings `'true'` or `'false'`, and add each header only once. Omitting either header leaves that tracking setting enabled by default. If you reuse a PHPMailer instance, clear its custom headers with `$mail->clearCustomHeaders()` before configuring the next message's headers.

See [SMTP tracking headers](/learn/sending/tracking#smtp-tracking-headers) for validation rules. These are Lettr control headers; arbitrary metadata headers and `X-MSYS-API` are not supported.
