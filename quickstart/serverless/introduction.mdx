...
The helper below retries network failures, server errors, rate limits, and an in-progress idempotent request. Validation errors and other client errors are thrown before the retry path, so they fail immediately:
