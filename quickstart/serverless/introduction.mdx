...
**Function timeout.** A send call normally completes in under a second, but the retry loop above can wait through several backoff intervals. Set the function timeout to cover the worst case rather than the happy path:
