...
This page covers sending templates, versioning, projects, and managing templates programmatically via the API.
...
If you only work with one project, you can omit the `projectId` parameter — templates are globally unique by slug across your team.

<Note>
  Projects help organize templates when you have multiple applications or brands sharing a Lettr account. Use the project ID to keep templates separate and avoid naming conflicts.
</Note>
...
Modify an existing template's content or metadata:

```php
use Lettr\Dto\Template\UpdateTemplateData;

$template = $lettr->templates()->update(
    slug: 'welcome-email',
...
Updates create a new template version in Lettr. The new version is not automatically published — you'll need to publish it in the Lettr dashboard to make it the active version.

<Note>
  Updating a template doesn't affect emails in flight or change the active version. This design prevents accidental changes to production templates.
</Note>
