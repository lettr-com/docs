---
title: FastAPI
description: "Integrate the Lettr Python SDK into a FastAPI app to send transactional emails, running the synchronous client in a worker thread so it never blocks the event loop."
---

<Note>
  New to the Lettr Python SDK? Start with the [Python Quickstart](/quickstart/python/quickstart) to learn the basics, then return here for FastAPI-specific integration patterns.
</Note>

Send transactional emails from your FastAPI applications using the official Lettr Python SDK.

<Warning>
  The Lettr Python SDK is **synchronous**. Calling it directly inside an `async def` route blocks FastAPI's event loop and will stall every other request in flight. Every example on this page wraps SDK calls in `asyncio.to_thread`, which hands the blocking call to a worker thread and awaits the result.
</Warning>
...
<Warning>
  The sender domain must be verified in your [Lettr dashboard](https://app.lettr.com/domains) before you can send emails. Sending from an unverified domain returns `400` with `error_code: unconfigured_domain`.
</Warning>
...
class EmailService:
    ...
    Wraps the synchronous Lettr client so FastAPI routes can await it
    without blocking the event loop.
    """

    def __init__(self):
        self.client = lettr.Lettr(settings.lettr_api_key)
...
Fan several sends out across worker threads with `asyncio.gather`. Note the API allows 3 requests/second per team, so bound this for large lists — and if every recipient gets identical content, prefer a single call with up to 50 addresses in `to` over N concurrent calls:
...
async def send_email(request: EmailRequest):
    try:
        response = await asyncio.to_thread(client.emails.send, ...)
        return {"success": True, "request_id": response.request_id}
