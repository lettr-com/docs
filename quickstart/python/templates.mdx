...
<Warning>
  **`purpose` is set once and cannot be changed.** Omit it and you get a transactional template, which a [campaign](/learn/campaigns/introduction) cannot use — and `purpose` is not accepted by the update endpoint, so the only remedy is to copy the template in the app. If the content is going to an audience, set it now. See [Transactional vs. Marketing Templates](/learn/templates/projects#transactional-vs-marketing-templates).
</Warning>
