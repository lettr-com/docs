...
description: "Troubleshoot failed domain verification in Lettr by fixing DNS propagation delays and misconfigured SPF, DKIM, and DMARC records"
---
...
<Note>
  Don't want to run `dig` yourself? Ask [Adamko](/learn/ai-assistant/troubleshooting-and-diagnostics#domain-verification-problems) — *"why isn't my domain verifying?"* runs the same live lookups, compares each record against what Lettr expects, and flags the classic mistakes below (wrong host, duplicate SPF, proxied CNAME) with a per-record fix.
</Note>
...
<Note>
  Verification is not a one-time event: once a domain is verified, Lettr's daily [DNS health check](/learn/settings/alerts#domain-dns-health-alerts) keeps watching it and alerts you if a record later stops resolving. If you see a **Not working** badge on a previously verified domain, fix the DNS record at your provider — the warning clears automatically on the next check.
</Note>
