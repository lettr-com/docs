...
description: "Read Lettr bounce data, tell hard from soft bounces, and use bounce categories to take the right action for each delivery failure"
---
...
<Tip>
  For a single problem recipient, [Adamko](/learn/ai-assistant/troubleshooting-and-diagnostics#a-specific-recipient-didnt-get-an-email) can do the bounce forensics for you — he reads the recipient's recent events, decodes the SMTP response, checks suppression state, and names the likely root cause. For a list-wide view, ask him to *"check my sending health"*.
</Tip>
