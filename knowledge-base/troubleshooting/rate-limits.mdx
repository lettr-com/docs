...
<Note>
  High-volume rate limits are available on Business and Enterprise plans. Contact [sales@lettr.com](mailto:sales@lettr.com) to discuss your volume needs.
</Note>
...
curl -s -w "\nHTTP Status: %{http_code}\n" \
  -X POST https://api.lettr.com/v1/emails \
...
async function sendToAll(recipients, emailPayload) {
  ...
  for (const chunk of chunks) {
    await sendWithBackoff("https://api.lettr.com/v1/emails", {
      ...emailPayload,
      to: chunk,
    });
...
| Plan | Monthly Quota |
|------|--------------|
| Starter | 10,000 emails |
| Growth | 100,000 emails |
| Business | 500,000 emails |
| Enterprise | Custom |

<Note>
  Exceeding your monthly quota may result in overage charges or sending being paused, depending on your plan. Check your current plan details in the Lettr dashboard under **Settings > Billing**.
</Note>
