...
### Use `campaign_id` for Marketing Emails

When sending marketing emails through the API, include a `campaign_id` to group related sends and track campaign-level metrics:
...
Transactional emails typically do not need a `campaign_id` since they are triggered individually.
