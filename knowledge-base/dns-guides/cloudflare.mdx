...
For a deeper look at how Domain Connect works under the hood — including the signed URL protocol, smart record skipping, and troubleshooting — see the dedicated [Domain Connect](/learn/domains/domain-connect) article.
...
- **DNS not hosted on Cloudflare** — If your domain's nameservers point to a different provider (even if you have a Cloudflare account), Domain Connect won't be available. Follow the manual steps below, or consult the [DNS guide](/knowledge-base/dns-guides) for your provider.
