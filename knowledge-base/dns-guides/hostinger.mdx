...
This guide walks you through adding the required DNS records for Lettr using Hostinger's DNS management. Hostinger is a web hosting provider that offers DNS hosting for domains managed through its [hPanel dashboard](https://www.hostinger.com).
...
Log in to your [Hostinger hPanel](https://www.hostinger.com) and navigate to **Domains** → **DNS**. Select the domain you want to configure.

{/* TODO: Add screenshot of Hostinger DNS management page */}
...
<AccordionGroup>
  <Accordion title="Domain registered with a different registrar?">
    ...
    at your registrar to Hostinger's nameservers shown in your domain configuration.

    <Warning>Until the nameservers are updated, any records you add in Hostinger will not resolve.</Warning>
  </Accordion>
</AccordionGroup>

{/* TODO: Add screenshot of Hostinger nameserver configuration inside the accordion above */}
...
1. In the DNS management page, scroll to the **CNAME records** section and click **Add new**.
2. Set **Name** to the **Hostname** value from Lettr (without your domain suffix). For example, paste only `mailing`.
3. Set **Target** to the **Value** from Lettr.
4. Set **TTL** to **3600**.
5. Click **Save**.


{/* TODO: Add screenshot of CNAME record in Hostinger */}
...
1. In the DNS management page, scroll to the **TXT records** section and click **Add new**.
...
3. Set **Content** to the **Value** from Lettr.
4. Set **TTL** to **3600**.
5. Click **Save**.


{/* TODO: Add screenshot of DKIM TXT record in Hostinger */}
...
3. Set **Content** to the **Value** from Lettr (e.g. `v=DMARC1;p=none;`).
4. Set **TTL** to **3600**.
5. Click **Save**.


{/* TODO: Add screenshot of DMARC TXT record in Hostinger */}
...
1. In the DNS management page, scroll to the **MX records** section and click **Add new**.
2. Set **Name** to the **Hostname** value from Lettr (without your domain suffix). For example, paste only `inbound`.
3. Set **Mail server** to `rx1.sparkpostmail.com`.
...
{/* TODO: Add screenshot of MX records in Hostinger */}
...
{/* TODO: Add screenshot of SSL Certificate CNAME record in Hostinger */}
...
{/* TODO: Add screenshot of Traffic CNAME record in Hostinger */}
...
{/* TODO: Add screenshot of Certificate Validation TXT record in Hostinger */}
...
{/* TODO: Add screenshot of Hostname Pre-validation TXT record in Hostinger */}
...
{/* TODO: Add screenshot of Domain Routing CNAME record in Hostinger */}
...
<AccordionGroup>
  <Accordion title="Records Not Propagating">
    Hostinger DNS may take up to 24 hours to propagate. If verification fails after adding records, check the following:
  ...
  <Accordion title="Nameservers Not Pointing to Hostinger">
    If your records are saved in Hostinger but not resolving, confirm that your domain registrar is pointing to Hostinger's
    ...
    The output should include Hostinger nameservers (e.g. `ns1.hostinger.com`, `ns2.hostinger.com`). If it shows a different
    provider's nameservers, update them at your domain registrar.
  </Accordion>
  <Accordion title="Wrong Record Type">
    A common mistake is adding a record to the wrong section in Hostinger's DNS page:
  ...
  <Accordion title="CNAME Conflict">
    Remove any existing A or CNAME records for the same hostname before adding new CNAME records. Duplicate records on
    the same hostname can cause verification failures.
  </Accordion>
  <Accordion title="DKIM Value Truncated">
    ...
    is displayed in the Content field. If it appears truncated, delete the record and re-add it, pasting the full value
    carefully.
  </Accordion>
