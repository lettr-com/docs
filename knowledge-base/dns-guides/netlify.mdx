...
This guide walks you through adding the required DNS records for Lettr using Netlify's DNS management. Netlify is a platform for deploying and hosting web applications that also provides DNS hosting for domains connected to your Netlify sites, managed through the [Netlify dashboard](https://app.netlify.com).
...
Log in to the [Netlify dashboard](https://app.netlify.com) and navigate to your site's **Domain management** settings. If your domain is managed through Netlify DNS, you can add records directly here.

{/* TODO: Add screenshot of Netlify Domain management page */}
...
<AccordionGroup>
  <Accordion title="Domain registered with a different registrar?">
    If you registered your domain elsewhere but want to manage DNS on Netlify, you must first **update the nameservers**
    at your registrar to Netlify's nameservers shown in your domain management settings.

    <Warning>Until the nameservers are updated, any records you add in Netlify will not resolve.</Warning>
  </Accordion>
</AccordionGroup>

{/* TODO: Add screenshot of Netlify nameserver configuration inside the accordion above */}

<Warning>
  If you have existing Netlify sites using subdomains that overlap with the subdomains you are configuring for Lettr (e.g.
  `mailing.company.com`), you may need to remove or reconfigure those site assignments before adding the DNS records.
</Warning>
...
1. Click **Add a custom record** in the DNS records section.
2. Set **Type** to **CNAME**.
...
{/* TODO: Add screenshot of CNAME record in Netlify */}
...
{/* TODO: Add screenshot of DKIM TXT record in Netlify */}
...
{/* TODO: Add screenshot of DMARC TXT record in Netlify */}
...
{/* TODO: Add screenshot of MX records in Netlify */}

<Info>
  Netlify MX records may require the priority in the value field (e.g. `10 rx1.sparkpostmail.com`). If you don't see a
  separate Priority field, include the priority number before the mail server value.
</Info>
...
{/* TODO: Add screenshot of SSL Certificate CNAME record in Netlify */}
...
{/* TODO: Add screenshot of Traffic CNAME record in Netlify */}
...
{/* TODO: Add screenshot of Certificate Validation TXT record in Netlify */}
...
{/* TODO: Add screenshot of Hostname Pre-validation TXT record in Netlify */}
...
{/* TODO: Add screenshot of Domain Routing CNAME record in Netlify */}
...
<AccordionGroup>
  <Accordion title="Root Domain CNAME Issues">
    Netlify uses ALIAS records for root domains. CNAME records cannot be added at the root domain. For email sending,
    use the subdomain approach or TXT SPF verification for root domains.
  </Accordion>
  <Accordion title="Subdomain Conflicts with Netlify Site">
    If a subdomain is already assigned to a Netlify site (e.g. `mailing.company.com` is a deployed site), Netlify may not
    allow you to add a CNAME record for that subdomain. Remove the site assignment first, or use a different subdomain for
    your Lettr sending domain.
  </Accordion>
  <Accordion title="Records Not Propagating">
    DNS propagation on Netlify typically takes 1–15 minutes, but can occasionally take longer. If verification fails after
    adding records, check the following:
  ...
  <Accordion title="DKIM Value Truncated">
    The DKIM TXT value is a long string. After creating the record, click on it in the DNS records list to confirm the full
    value was stored. If it appears truncated, delete the record and re-add it, pasting the full value carefully.
  </Accordion>
