...
Log in to the [DreamHost Panel](https://panel.dreamhost.com) and navigate to **Domains** → **Manage Domains**. Select your domain, then click **DNS** to manage its records.

{/* TODO: Add screenshot of DreamHost DNS management page */}

<Tip>
  If your domain was purchased through DreamHost or your nameservers already point to DreamHost, skip nameserver
  configuration entirely. Your domain is already using DreamHost's nameservers by default — proceed directly to adding the
  DNS records below.
</Tip>

<AccordionGroup>
  <Accordion title="Domain registered with a different registrar?">
    If you registered your domain elsewhere but want to manage DNS on DreamHost, you must first **update the nameservers**
    at your registrar to DreamHost's nameservers. You can find these in the domain's configuration settings in the DreamHost Panel.

    <Warning>Until the nameservers are updated, any records you add in DreamHost will not resolve.</Warning>
  </Accordion>
</AccordionGroup>

{/* TODO: Add screenshot of DreamHost nameserver configuration inside the accordion above */}
...
5. Click **Add this record**.


{/* TODO: Add screenshot of CNAME record in DreamHost DNS */}

<Warning>
  Do not add a CNAME record at the root of your domain (e.g. `company.com`). DreamHost does not support CNAME records at the
  root, and doing so will cause errors. Always use a subdomain for your sending domain.
</Warning>
...
{/* TODO: Add screenshot of DKIM TXT record in DreamHost DNS */}
...
{/* TODO: Add screenshot of DMARC TXT record in DreamHost DNS */}
...
3. Set **Type** to **MX**.
4. Set **Value** to the first MX record value from Lettr (e.g. `rx1.sparkpostmail.com`).
5. Set **Priority** to `10`.
6. Click **Add this record**.
...
{/* TODO: Add screenshot of MX records in DreamHost DNS */}

<Warning>
  Adding MX records to a domain or subdomain will route all incoming email for that address to Lettr. Use a dedicated
  subdomain (e.g. `inbound.company.com`) to avoid interfering with DreamHost's own email services.
</Warning>
...
{/* TODO: Add screenshot of SSL Certificate CNAME record in DreamHost DNS */}
...
{/* TODO: Add screenshot of Traffic CNAME record in DreamHost DNS */}
...
{/* TODO: Add screenshot of Certificate Validation TXT record in DreamHost DNS */}
...
{/* TODO: Add screenshot of Hostname Pre-validation TXT record in DreamHost DNS */}
...
{/* TODO: Add screenshot of Domain Routing CNAME record in DreamHost DNS */}
...
<AccordionGroup>
  <Accordion title="Records Not Propagating">
    DreamHost DNS propagation typically takes 15–60 minutes. If verification fails after adding records, check the following:
  ...
  <Accordion title="CNAME at Root Domain Not Supported">
    DreamHost does not support CNAME records at the root of a domain. You must use a subdomain for your sending, tracking,
    and storage domains. This is also a general DNS limitation — CNAME records cannot coexist with other record types at the
    same name.
  </Accordion>
  ...
  <Accordion title="DKIM Value Truncated">
    The DKIM TXT value is a long string. After saving the record, re-open the DNS management page and confirm the full value
    is displayed. If it appears truncated, delete the record and re-add it, pasting the full value carefully.
  </Accordion>
